...
UINT  ux_dcd_synergy_uninitialize (ULONG dcd_io)
{
...
#endif
    {
        ...
        _ux_system_error_handler((UINT)UX_SYSTEM_LEVEL_THREAD,
                                 (UINT)UX_SYSTEM_CONTEXT_DCD, (UINT)UX_CONTROLLER_UNINIT_FAILED);

        /* If trace is enabled, insert this event into the trace buffer.  */
        UX_TRACE_IN_LINE_INSERT(UX_TRACE_ERROR, UX_CONTROLLER_UNINIT_FAILED, 0, 0, 0, UX_TRACE_ERRORS, 0, 0)

        /* Return failure status  */
        return (UINT)UX_CONTROLLER_UNINIT_FAILED;
    }
...
UINT  ux_dcd_synergy_uninitialize_transfer_support (ULONG dcd_io)
{
    ...
    if (NULL != dcd_synergy->ux_dcd_synergy_transfer.ux_synergy_transfer_tx)
    {
        ...
        {
            status = (UINT)UX_CONTROLLER_UNINIT_FAILED;
        }
