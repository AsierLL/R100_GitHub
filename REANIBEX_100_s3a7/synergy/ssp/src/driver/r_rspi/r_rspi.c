...
/*************************************************************************************************************//**
 * @brief   This function is the ISR function for RSPI idel interrupts.
...
void spi_rxi_isr (void)
{
    ...
    {
        /* Transfer complete. */
        /* slave can return immediately. master needs to wait on idle ortx complete before callback  */
        if ((HW_RSPI_MasterModeCheck(p_ctrl->p_reg)) && (HW_RSPI_IdleCheck(p_ctrl->p_reg)))
        {
            /* If master mode enable and wait for transmit end ISR. */
            NVIC_EnableIRQ(p_ctrl->tei_irq);
        }
        else
        {
...
void spi_txi_isr (void)
{
    ...
    if (p_ctrl->tx_count == p_ctrl->xfr_length)
    {
        ...
        /* Check if we are master or slave since idle interrupt behaves differently */
        if (HW_RSPI_MasterModeCheck (p_ctrl->p_reg))
        {
        ...
        {
            /* In Slave mode,the TEI interrupt will not to be handled, as slave can return immediately
             * after last data has been transmitted, and as same stated in the user hardware manual
             * hence complete the transaction here by calling user callback function. */
            if ((NULL != p_ctrl->p_callback))
            {
                spi_callback_args_t rspi_cb_data;
                rspi_cb_data.channel            = p_ctrl->channel;
                                      ...
                                      uint32_t               const length )
{
    ...
    {
        /* Restore source address mode to default mode. */
 ...
 * @param[in] Pointer to the control block..
 * @param[in] bitwidth    Size of data for each transaction.
 * @note      the parameter check must be held by HLD
 ********************************************************************************************************************/
static void rspi_dtc_close_reconfig_p_info(rspi_instance_ctrl_t            * const p_ctrl,
                                     spi_bit_width_t const         bit_width)
{
    uint32_t tx_dummy_address = 0U;
    uint32_t rx_dummy_address = 0U;
